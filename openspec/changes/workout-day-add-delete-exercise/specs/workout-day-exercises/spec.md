...
El sistema SHALL permitir agregar un ejercicio nuevo a un día de workout existente desde la página `/workout/[date]`, creando al menos una primera serie para ese ejercicio sin regenerar el plan semanal.

#### Scenario: Agregar ejercicio con sesión existente en el día
- **WHEN** el usuario abre el modal de agregar ejercicio desde el botón **+** al pie de la lista de `/workout/[date]`, completa nombre, unidad y primera serie, y confirma
...
- **WHEN** no existe ninguna `WorkoutSession` cuyo `startedAt` caiga en el día `date` y el usuario agrega un ejercicio
- **THEN** el sistema crea una sesión con `startedAt` al inicio (UTC 00:00:00) de ese `date`, le asigna la nueva serie al ejercicio, y la tarjeta aparece en `/workout/[date]`
...
- **WHEN** se agrega un ejercicio y la petición falla por `!navigator.onLine` o `TypeError`
- **THEN** el sistema encola la mutación, inserta el set en el cache local del día con un id temporal, y muestra un mensaje de sincronización pendiente
