...
El sistema SHALL permitir agregar un ejercicio nuevo a un día de workout existente desde la página `/workout/[date]`, creando todas las series configuradas por el usuario en una sola operación sin regenerar el plan semanal. Cuando exista una propuesta recalculada activa con nivel distinto de `Sin cambios`, el sistema SHALL presentar primero el banner de recálculo (ver capability `session-recalculation-confirmation`) antes de habilitar el botón **+** de agregar ejercicio, de forma que el usuario resuelva la propuesta antes de añadir trabajo nuevo.
...
#### Scenario: Botón + bloqueado hasta resolver recálculo
- **WHEN** existe una propuesta recalculada activa con nivel distinto de `Sin cambios` y el usuario aún no ha aceptado ni rechazado
- **THEN** el botón **+** de agregar ejercicio permanece deshabilitado y resalta el banner de recálculo, forzando al usuario a decidir la versión a ejecutar antes de añadir trabajo nuevo

### Requirement: Persistencia y cola offline de las mutaciones de ejercicio del día
Las operaciones de agregar y eliminar ejercicio sobre un día MUST encolarse en la offline queue cuando no hay conexión, aplicarse optimistamente sobre el cache local, y sincronizarse al reconectar, con la misma UX (mensajes de `syncError`) que el resto de las mutaciones de workout. La aceptación/rechazo de la propuesta recalculada (capability `session-recalculation-confirmation`) se trata como una mutación más del día y sigue el mismo camino de sincronización.
...
#### Scenario: Elección de recálculo encolada offline
- **WHEN** el usuario acepta o rechaza la propuesta recalculada y la petición falla por `!navigator.onLine`
- **THEN** el sistema encola la mutación de `recalc/accept`, aplica el cambio de `recalculationVersion` optimistamente en la UI, y al reconectar envía el POST al servidor con el mismo patrón de `syncError` que el resto de las mutaciones del día

### Requirement: Coherencia gráfica de los nuevos controles
Los nuevos botones (agregar ejercicio, eliminar por tarjeta y los botones del banner de recálculo: **Aceptar** / **Seguir plan original**) MUST respetar la coherencia gráfica del resto de la aplicación: reuso de `Modal`/`ConfirmDialog`, clases `btn-dark`/`btn-accent`/`panel-soft`, e iconos de `lucide-react`. El banner de recálculo usa `panel-soft` y el color accent `#d6ff43` para resaltar subidas y `gray-400` para bajadas.
...
#### Scenario: Botones del banner de recálculo
- **WHEN** se renderiza el banner de propuesta recalculada
- **THEN** el botón **Aceptar** usa `btn-accent` y el botón **Seguir plan original** usa `btn-dark`, ambos respetando el spacing y tipografía del resto de la app